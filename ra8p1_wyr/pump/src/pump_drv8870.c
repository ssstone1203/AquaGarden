/*******************************************************************************************************************//**
 * DRV8870/DRV8871 brushed DC pump driver — hardware notes
 *
 ...
 *   - A buck regulator (e.g. 12 V -> 5 V or 3.3 V) powers the RA8P1 only; do NOT feed 12 V to the MCU.
 ...
 *   - CPKHMI-RA8P1：PUMP_VREF (P206) → DRV8870 VREF（DRV8871 模块无 VREF 脚时可不接）。
 *
...
/** CPKHMI-RA8P1：P206 -> DRV8870 VREF（RASC user label PUMP_VREF） */
#define PUMP_VREF_PIN   PUMP_VREF

/** P105 -> DRV8870/DRV8871 IN1（RASC user label PUMP_IN1） */
#define PUMP_IN1_PIN    PUMP_IN1

/** GPT output pin select for IN2 (P104 / GTIOC1B) */
...
{
    return g_timer0_cfg.period_counts;
}

/**
 * IN1=1、IN2=1 慢衰减制动（DRV8870 表），不拉双低 → 不会进休眠。
 * 用 GPT 让 GTIOCB 整周期为高；若板子 GPT 仍异常，请用 run_forward_dc 验证直流路径。
 */
static void pump_drv8870_brake_holding(void)
{
    ...
    (void) g_timer0.p_api->dutyCycleSet(g_timer0.p_ctrl, brake_min_low_counts, PUMP_PWM_PIN);
    (void) g_timer0.p_api->start(g_timer0.p_ctrl);
...
void pump_drv8870_init(void)
{
    ...
    (void) g_timer0.p_api->open(g_timer0.p_ctrl, g_timer0.p_cfg);
    (void) g_timer0.p_api->stop(g_timer0.p_ctrl);
}
...
void pump_drv8870_set_power(uint8_t power_percent)
{
    ...
    (void) g_ioport.p_api->pinWrite(&g_ioport_ctrl, PUMP_IN1_PIN, BSP_IO_LEVEL_HIGH);
    (void) g_timer0.p_api->dutyCycleSet(g_timer0.p_ctrl, duty_compare, PUMP_PWM_PIN);
    (void) g_timer0.p_api->start(g_timer0.p_ctrl);
    pump_drv8870_after_drive_asserted();
}
