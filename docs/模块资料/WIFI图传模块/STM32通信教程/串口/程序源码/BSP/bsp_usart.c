...
//USART1 ---- 与PC通信，输出调试信息 USART1 ---- communicate with PC and output debugging information
...
//	NVIC_InitTypeDef NVIC_InitStructure; 
	...
	//串口1只做发送数据功能，所以不使用中断了 Serial port 1 only sends data, so no interrupt is used.
...
int fputc(int ch, FILE *f)
{
	...
	/* 等待发送完毕 Waiting for sending to complete */
	while (USART_GetFlagStatus(USART1, USART_FLAG_TXE) == RESET)
		;
	return (ch);
}

///重定向c库函数scanf到串口，重写向后可使用scanf、getchar等函数 Redirect the C library function scanf to the serial port, rewrite it to use scanf, getchar and other functions
int fgetc(FILE *f)
{
	/* 等待串口输入数据 等待串口输入数据 Waiting for serial port input data Waiting for serial port input data*/
...
void USART2_init(u32 baudrate)
{
	...
	USART_ITConfig(USART2, USART_IT_RXNE, ENABLE); //开启接收中断 Enable receive interrupt
