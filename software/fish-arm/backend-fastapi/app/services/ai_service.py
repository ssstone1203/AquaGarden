...
async def analyze(snapshot: SensorSnapshot, from_hardware: bool) -> dict[str, Any]:
    ...
    system = "你是水族箱与智慧盆栽一体化生态系统的助手。根据传感器数据用中文给出简短、可执行的建议，控制在约 200 字内，分「状态」「风险」「建议」三层表述。"
...
async def chat(snapshot: SensorSnapshot, from_hardware: bool, message: str, history: list[AiChatMessage]) -> dict[str, Any]:
    ...
    system = "你是水族箱与智慧盆栽一体化生态系统的对话助手。根据实时传感器状态和上下文，用中文回答用户问题；建议要具体、可执行。不要编造未提供的数据。"
...
async def _call_llm(snapshot: SensorSnapshot, from_hardware: bool, system: str, user: str, history: list[dict[str, str]]) -> dict[str, Any]:
    ...
    try:
        ...
        return _fallback(snapshot, from_hardware, "error", f"大模型调用异常：{str(exc)[:220]} 以下为本地规则回退。")
...
def _sensor_context(s: SensorSnapshot, from_hardware: bool) -> str:
    ...
    return f"数据来源：{source}\n水温 {s.water_temp:.1f} °C，气温 {s.air_temp:.1f} °C，空气湿度 {s.air_humidity:.1f} %RH，水质综合指数 {s.wqi:.0f} / 100，土壤湿度 {s.soil_moisture:.0f} %。"
...
def _rule_based(s: SensorSnapshot, from_hardware: bool) -> str:
    ...
    parts.append("【风险】水质指数偏低，需关注过滤、投喂量与换水频率。" if s.wqi < 50 else "【风险】水质指数良好，继续保持定期维护即可。" if s.wqi >= 80 else "【风险】水质中等，建议观察浑浊度与鱼类摄食行为。")
    ...
    parts.append("【建议】若长期离线，请检查串口桥接脚本与 MCU 帧是否持续 POST 到 /api/sensors/ingest。")
    return "".join(parts)
