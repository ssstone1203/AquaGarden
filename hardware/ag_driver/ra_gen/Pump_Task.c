...
#if 1
                static StaticTask_t Pump_Task_memory;
                #if defined(__ARMCC_VERSION)           /* AC6 compiler */
                static uint8_t Pump_Task_stack[1024] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.thread") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
                #else
                static uint8_t Pump_Task_stack[1024] BSP_PLACE_IN_SECTION(BSP_UNINIT_SECTION_PREFIX ".stack.Pump_Task") BSP_ALIGN_VARIABLE(BSP_STACK_ALIGNMENT);
                #endif
                #endif
                TaskHandle_t Pump_Task;
                void Pump_Task_create(void);
                static void Pump_Task_func(void * pvParameters);
...
const timer_cfg_t g_pump_timer_cfg =
{
    ...
    /* Actual period: 0.00004 seconds. Actual duty: 50%. */ .period_counts = (uint32_t) 0xfa0, .duty_cycle_counts = 0x7d0, .source_div = (timer_source_div_t)0,
    .channel             = 4,
    .p_callback          = NULL,
...
extern uint32_t g_fsp_common_thread_count;
                ...
                const rm_freertos_port_parameters_t Pump_Task_parameters =
                {
                    .p_context = (void *) NULL,
                };
                ...
                void Pump_Task_create (void)
                {
                    ...
                    #if 1
                    Pump_Task = xTaskCreateStatic(
                    #else
                    BaseType_t Pump_Task_create_err = xTaskCreate(
                    #endif
                        Pump_Task_func,
                        (const char *)"Pump_Driver",
                        1024/4, // In words, not bytes
                        (void *) &Pump_Task_parameters, //pvParameters
                        1,
                        #if 1
                        (StackType_t *)&Pump_Task_stack,
                        (StaticTask_t *)&Pump_Task_memory
                        #else
                        & Pump_Task
                        #endif
                    );
                    ...
                    if (NULL == Pump_Task)
                    {
                        rtos_startup_err_callback(Pump_Task, 0);
                    }
                    #else
                    if (pdPASS != Pump_Task_create_err)
                    {
                        rtos_startup_err_callback(Pump_Task, 0);
                    }
                    #endif
