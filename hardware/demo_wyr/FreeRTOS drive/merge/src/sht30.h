...
/*
 * 显示用饱和：输出始终先按手册公式换算，再限制在规格量程内（低温/超温时显示为 −40 °C 或 125 °C，
 * 湿度为 0 % 或 100 %）。原始 tick 仍在 g_sht30_temperature_raw / g_sht30_humidity_raw。
 */
 ...
 * 按 Sensirion 公式换算为 °C、%RH，再饱和到规格显示范围（SHT30_TEMP_C_SPEC_*、SHT30_RH_PCT_SPEC_*）；°F 由饱和后的 °C 换算。
