...
#define PRESSURE_SENSOR_SCAN_TIMEOUT_LOOPS (200000U)
...
static fsp_err_t pressure_do_single_scan(void)
{
    ...
    /*
     * RA generated ADC config in this project uses CONTINUOUS_SCAN mode.
     * In this mode scanStatus can stay IN_PROGRESS for a long time, and polling it
     * in a tight loop will starve lower-priority RTOS tasks.
     * Give ADC a short settle window, then read channels directly.
     */
...
fsp_err_t pressure_sensor_init(void)
{
    ...
    adc_channel_cfg_t channel_cfg = *((adc_channel_cfg_t const *) g_adc.p_channel_cfg);
    channel_cfg.scan_mask = (1UL << ADC_CHANNEL_0) | (1UL << ADC_CHANNEL_1) | (1UL << ADC_CHANNEL_2);
    channel_cfg.scan_mask_group_b = 0U;
    err = g_adc.p_api->scanCfg(g_adc.p_ctrl, &channel_cfg);
    if (FSP_SUCCESS != err)
    {
        return err;
    }
