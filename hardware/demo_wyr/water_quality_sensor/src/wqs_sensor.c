...
{
    // Open UART instance once. Without this, R_SCI_UART_Read/Write will fail.
...
void WQS_CmdSend(const wqs_cmd_t* wqs_cmd, wqs_cmd_type_e wqs_cmd_type)
{
    ...
    switch (wqs_cmd_type)
    {
        case WQS_CMD_TYPE_DETECT:
            (void)R_SCI_UART_Write(&g_uart0_ctrl, wqs_cmd->wqs_cmd_detect, 10U);
            break;

        case WQS_CMD_TYPE_CALIBRATE:
            (void)R_SCI_UART_Write(&g_uart0_ctrl, wqs_cmd->wqs_cmd_calibrate, 10U);
            break;
...
bool WQS_InfoGet(volatile wqs_info_t* wqs_info)
{
    ...
    fsp_err_t err = R_SCI_UART_Read(&g_uart0_ctrl, (uint8_t *)wqs_info->wqs_info_raw, WQS_INFO_LEN);
    if (err != FSP_SUCCESS)
    {
