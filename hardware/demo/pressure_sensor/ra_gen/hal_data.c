...
#include "hal_data.h"
adc_instance_ctrl_t g_ps_adc_ctrl;
const adc_extended_cfg_t g_ps_adc_cfg_extend =
...
{ .unit = 0, .mode = ADC_MODE_CONTINUOUS_SCAN, .resolution = ADC_RESOLUTION_12_BIT, .alignment =
  ...
  .p_callback = PS_ScanCpltCallback,
...
#if ((0) | (0))
const adc_window_cfg_t g_ps_adc_window_cfg =
{
    .compare_mask        =  0,
    .compare_mode_mask   =  0,
...
#endif
const adc_channel_cfg_t g_ps_adc_channel_cfg =
{ .scan_mask = ADC_MASK_CHANNEL_2 | ADC_MASK_CHANNEL_7 | 0, .scan_mask_group_b = 0, .priority_group_a =
          ADC_GROUP_A_PRIORITY_OFF,
  .add_mask = 0, .sample_hold_mask = 0, .sample_hold_states = 24,
#if ((0) | (0))
    .p_window_cfg        = (adc_window_cfg_t *) &g_ps_adc_window_cfg,
#else
  .p_window_cfg = NULL,
#endif
        };
/* Instance structure to use this module. */
const adc_instance_t g_ps_adc =
{ .p_ctrl = &g_ps_adc_ctrl, .p_cfg = &g_ps_adc_cfg, .p_channel_cfg = &g_ps_adc_channel_cfg, .p_api = &g_adc_on_adc };
