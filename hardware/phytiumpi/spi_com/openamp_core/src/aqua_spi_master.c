 ...
 *   - SPI Mode 0：CPOL=0/CPHA=0（与 v1 spidev、RA6E2 FSP 一致）
...
int aqua_spi_master_init(void)
{
    ...
    cfg.cpha         = FSPIM_CPHA_1_EDGE;    /* CPHA=0 → Mode 0 */
    ...
    s_ready = 1;
    AQUA_I("FSPI%u inited @ %u Hz, Mode 0, 8-bit, POLL", AQUA_SPI_ID, AQUA_SPI_SCLK_HZ);
    return 0;
}
