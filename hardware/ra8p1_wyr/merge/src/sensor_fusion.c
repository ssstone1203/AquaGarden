...
volatile fsp_err_t g_tds_last_err    = FSP_ERR_NOT_INITIALIZED;
volatile fsp_err_t g_uwt_last_err    = FSP_ERR_NOT_INITIALIZED;
volatile uint32_t  g_air_retry_count = 0U;
volatile uint32_t  g_tds_retry_count = 0U;
volatile uint32_t  g_uwt_retry_count = 0U;

volatile uint8_t g_pump_enable               = 0U;
...
volatile float    g_jscope_tds_ppm        = 0.0F;
volatile uint8_t  g_jscope_tds_wqi        = 0U;
...
{
    /* Map TDS ppm to 0-100 WQI-like index for upstream UART frame byte 17. */
    if (tds_ppm <= 0.0F)
    {
        return 0U;
    }
